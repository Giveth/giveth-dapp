...
var index$4 = (input, reviver, filename) => {
...
module.exports = function requireFromString(code, filename, opts) {
	...
	var paths = Module._nodeModulePaths(path$$1.dirname(filename));
...
const requireFromString$1 = index$12;
...
function inferFormat(filepath        )          {
  switch (path$3.extname(filepath)) {
    case '.js':
      return 'js';
    case '.json':
      return 'json';
...
var getDirectory$1 = function getDirectory(
  ...
  if (sync === true) {
    return isDirectory.sync(filepath) ? filepath : path$4.dirname(filepath);
  }
  ...
  return new Promise((resolve, reject) => {
    return isDirectory(filepath, (err, filepathIsDirectory) => {
      ...
      return resolve(filepathIsDirectory ? filepath : path$4.dirname(filepath));
    });
  });
};
