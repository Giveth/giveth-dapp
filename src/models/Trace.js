...
import TraceItemModel from './TraceItem';

/**
 * The DApp Trace model
 */
export default class Trace extends BasicModel {
  constructor(data) {
    super(data);
  ...
  toFeathers(txHash) {
    ...
    if (!this.id) trace.txHash = txHash;
  ...
  * */
  ...
  static get PROPOSED() {
    return Trace.statuses.PROPOSED;
  }

  static get REJECTED() {
    return Trace.statuses.REJECTED;
  }

  static get PENDING() {
    return Trace.statuses.PENDING;
  }

  static get IN_PROGRESS() {
    return Trace.statuses.IN_PROGRESS;
  }

  static get NEEDS_REVIEW() {
    return Trace.statuses.NEEDS_REVIEW;
  }

  static get COMPLETED() {
    return Trace.statuses.COMPLETED;
  }

  static get CANCELED() {
    return Trace.statuses.CANCELED;
  }

  static get PAYING() {
    return Trace.statuses.PAYING;
  }

  static get PAID() {
    return Trace.statuses.PAID;
  }

  static get FAILED() {
    return Trace.statuses.FAILED;
  }

  static get ARCHIVED() {
    return Trace.statuses.ARCHIVED;
  }
  ...
  static get EXPENSETYPE() {
    return Trace.formTypes.EXPENSE;
  }

  static get PAYMENTTYPE() {
    return Trace.formTypes.PAYMENT;
  }
  ...
  set status(value) {
    this.checkValue(value, Object.values(Trace.statuses), 'status');
    this._status = value;
  }
  ...
  set items(value) {
    value.forEach(item => {
      this.checkInstanceOf(item, TraceItemModel, 'items');
    });
  ...
  // It has meaning just for capped traces
  ...
  get isArchived() {
    return this._status === Trace.ARCHIVED;
  }
  ...
  canUserAcceptRejectProposal(user) {
    return (
      ...
      this.status === Trace.PROPOSED
    );
  }

  canUserRepropose(user) {
    return user && this.ownerAddress === user.address && this.status === Trace.REJECTED;
  }
  ...
  canUserDelete(user) {
    return (
      ...
      [Trace.REJECTED, Trace.PROPOSED].includes(this.status)
    );
  }
  ...
  canUserCancel(user) {
    return (
      ...
      [Trace.IN_PROGRESS, Trace.NEEDS_REVIEW].includes(this.status) &&
      this.mined
    );
  }
  ...
  canUserApproveRejectCompletion(user) {
    return (
      user &&
      this.status === Trace.NEEDS_REVIEW &&
  ...
  canUserEdit(user) {
    return (
      ...
      ((this.ownerAddress === user.address &&
        [Trace.PROPOSED, Trace.REJECTED, Trace.IN_PROGRESS, Trace.NEEDS_REVIEW].includes(
          this.status,
        )) ||
  ...
  canUserArchive(user) {
    return (
      user &&
      (!this.isCapped || this.fullyFunded === false) && // There is no reason to archive fully funded capped trace
      this.hasRecipient &&
      ((this.status === Trace.IN_PROGRESS && !this.hasReviewer) ||
        (this.status === Trace.COMPLETED && !this.isArchived)) &&
