...
function MyCampaigns() {
  ...
  const cleanup = () => CampaignService.unsubscribe();
                  ...
                  <table className="table table-responsive table-striped table-hover">
                    <thead>
                      <tr>
                        ...
                        <th className="td-confirmations">{isPendingCampaign && 'Confirmations'}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {campaigns.data.map(c => (
                        <tr key={c.id} className={c.status === Campaign.PENDING ? 'pending' : ''}>
                          ...
                          <td className="td-name">
                            <Link to={`/campaign/${c.slug}`}>{getTruncatedText(c.title, 45)}</Link>
